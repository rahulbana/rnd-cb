...
def http_get_text(url: str, headers: Optional[dict] = None, timeout: int = 30) -> str:
    return http_get(url, headers=headers, timeout=timeout).decode("utf-8", "replace")
...
class M3U8Downloader:
    ...
    def resolve_media_playlist(
        ...
        text = http_get_text(url, self.headers, self.timeout)
    ...
    def _get_key(self, uri: str) -> bytes:
        if uri not in self._key_cache:
            self._key_cache[uri] = http_get(uri, self.headers, self.timeout)
    ...
    def _fetch_segment(self, segment: Segment, idx: int, tmp_dir: str) -> str:
        ...
        for attempt in range(self.max_retries):
            try:
                data = http_get(segment.url, self.headers, self.timeout)
