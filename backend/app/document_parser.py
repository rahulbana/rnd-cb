...
Supported formats: PDF, DOC/DOCX, PPT/PPTX, TXT and other plain-text files
(md, csv, rtf-as-text, etc.). Everything runs in-memory; nothing is written
...
def extract_text(filename: str, data: bytes) -> str:
    ...
    if ext in _PDF_EXTS:
        ...
        if not text.strip():
            raise UnsupportedFileError(
                ...
                "PDF, DOCX, PPTX, and plain-text files."
            )
    ...
    if not text.strip():
        raise UnsupportedFileError(
            ...
            "a scanned image or an empty file."
        )
    return text
